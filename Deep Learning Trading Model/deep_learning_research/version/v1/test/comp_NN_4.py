...
def nn_strategy(row):
    ...
    if predictions[row['stock']] is None or stock_transaction_info[row['stock']]['ticks_elapsed'] % 20 == 0:
        ...
        percentage_difference = abs(
            (np.mean(np.array([*[row['price']] * 100]) - np.array(predictions[row['stock']])) / row['price']) * 100)
        if difference >= 100:
        ...
        if selling_volume_skew_fn == 'No Purchase' or buying_volume_skew_fn == 'No Purchase':
            ...
            weighted_buying_volume = buying_volume_average_fn
            weighted_selling_volume = selling_volume_average_fn
            
            # Calculate the confidence intervals for the model confidence
            confidence_interval_buy = (
                model_confidence_buy - 1.97, model_confidence_buy + 1.97)
            confidence_interval_sell = (
                model_confidence_sell - 1.97, model_confidence_sell + 1.97)
            
            # Calculate the adjusted volume based on the model confidence and risk factors
            max_confidence = max(abs(confidence_interval_buy[1]), abs(confidence_interval_sell[1]))
            min_risk = min_risk / 100
            max_risk = max_risk / 100
            
            adjusted_volume = min(weighted_buying_volume, weighted_selling_volume) * (1 - max_confidence / 100) * (
                    1 - max_risk) * (1 + min_risk)
            
            # Calculate the skew factor based on buying and selling volume
            skew_factor = 1 + selling_volume_skew_fn + buying_volume_skew_fn
            
            # Divide the adjusted volume by 10000 and apply the skew factor
            tradeable_volume = round(adjusted_volume * skew_factor / 10000)
...
with open(tick_data_path, 'r') as tick_data:
    tick_data.readline()
    while True:
        ...
        if current_row[1] not in stop_loss:
            stop_loss[current_row[1]] = {
                'long_stop_loss': None,
        ...
        print(
            f'(\x1b[34m{memory_usage:.2f}\x1b[0m) Index: \x1b[34m{current_row_ta["idx"]}\x1b[0m \x1b[0m(\x1b[32m{(current_ms_time / 14400000) * 100:.2f}\x1b[0m%), Time: \x1b[33m{(time.time() - start_time):.2f}s\x1b[0m',
            end='\r'
        )
        ...
        if stop_loss[current_row_ta["stock"]]["short_stop_loss"] is not None and \
            ...
            if current_row_ta["price"] >= stop_loss[current_row_ta["stock"]]["short_stop_loss"]:
                ...
                if stop_loss[current_row_ta['stock']]['short_stop_loss_volume'] <= 0:
                    ...
                    order_time.flush()
                    continue
                buy(stop_loss[current_row_ta["stock"]]["short_stop_loss_volume"], current_row_ta, order_time)
                print(
                    f"Fulfilled short stop loss order, {current_row_ta['stock'], stop_loss[current_row_ta['stock']]['short_stop_loss_volume']}")
                stop_loss[current_row_ta["stock"]]["short_stop_loss"] = None
                stop_loss[current_row_ta["stock"]]["short_stop_loss_volume"] = None
                continue
        ...
        if stop_loss[current_row_ta["stock"]]["long_stop_loss"] is not None and \
            ...
            if current_row_ta["price"] <= stop_loss[current_row_ta["stock"]]["long_stop_loss"]:
                ...
                if stop_loss[current_row_ta['stock']]['long_stop_loss_volume'] <= 0:
                    ...
                    order_time.flush()
                    continue
                sell(stop_loss[current_row_ta["stock"]]["long_stop_loss_volume"], current_row_ta, order_time)
        ...
        if len(stock_ta[current_row_ta["stock"]]["window"]) > 30:
            for key in stock_ta[current_row_ta["stock"]].keys():
                if len(stock_ta[current_row_ta["stock"]][key]) > 30:
                    stock_ta[current_row_ta["stock"]][key].pop(0)


        # Start feature engineering after 5 minutes
        ...
        if get_ms(current_row_ta["time"]) < (900000*2):
            if (stock_transaction_info[current_row_ta["stock"]]["time_last_bought"] is None) \
                    or (((get_ms(current_row_ta["time"]) - stock_transaction_info[current_row_ta["stock"]][
                "time_last_bought"]) > 60001)
                        and (stock_transaction_info[current_row_ta["stock"]]["buy_transactions"] < 3)):
                buy(1, current_row_ta, order_time)
                continue
            if (stock_transaction_info[current_row_ta["stock"]]["time_last_sold"] is None) or (
                    (get_ms(current_row_ta["time"]) -
